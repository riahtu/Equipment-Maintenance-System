...
session_start();
 date_default_timezone_set('Asia/Kuala_lumpur');
   ...
   $today = date("Y-m-d");
    $cyear = date("Y");
 require('db_ems.php');
echo "<div style='margin-top:20px;text-align:left;color:#007CB9;font-weight:bold;'>List of Purchase Order Return</div>";
$result1 = mysql_query("SELECT * from mprv_reverse_header where  status = 'C'	");
if (!mysql_num_rows($result1) == 0 )
{
	while($row1 = mysql_fetch_array($result1))
    {
		...
		echo "<div class='pp_m2_2x_choose' docno='$row1[docno]' style='cursor:pointer;border:1px solid #63B1B1;width:250px;min-height:100px;border-radius:5px;display: inline-block;margin:20px;'>";
		echo "<table class='pr_ticket' style='width:100%;text-align:left; border-collapse: collapse;'>";
		echo "<tr><td colspan=2 style='border-bottom:1px solid #808080;height:30px;font-size:16px;color:#black;font-weight:bold;background-color:#b3ffb3;text-align:center;'>$row1[docno]</td></tr>";
