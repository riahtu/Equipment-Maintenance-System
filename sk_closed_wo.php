...
session_start();
 require('db_ems.php');
echo "<div style='display: inline-block;width:80%;font-size:20px;text-align:left;font-weight:bold;color:#404000;'>Closed Requisitions</div>";
echo "<div style='display: inline-block;width:20%;'>Monitoring ON<input type='hidden' id='pick_order_monitoring' value='ON'/> </div>";
echo "<p id='changeList' class='changeList' program='sk_closed_wo2.php'>Change List Style</p>";
...
echo "<td style='width:110px;height:30px;font-weight:bold;border:1px solid #ACACAC;border-right:0px;padding-left:3px;'>Closed Time</td>";
...
if (!mysql_num_rows($result1) == 0 )
{
	...
	while($row1 = mysql_fetch_array($result1))
    {
		...
		echo "<td style='width:110px;border:1px solid #ACACAC;border-right:0px;border-top:0px;padding-left:3px;'>$closedtime</td>";
		...
		echo "<div id='pick_wo_closed' class='icon_part_change' title='View' workorderid='$row1[workorderid]'></div>";
