<?php
 ...
 require('db_ems.php');
 $docno = $_GET[docno];
$result1 = mysql_query("SELECT * from mpr_receipt_header where docno = '$_GET[docno]' and status = 'P'");
if (!mysql_num_rows($result1) == 0 )
{
	...
	$updatetime = mysql_result($result1, 0, 'updatetime');
	$updatetime = substr($updatetime, 8,2).'-'.substr($updatetime, 5,2).'-'.substr($updatetime, 0,4);
}
$suppliername = '';
...
echo "<tr><td style='width:150px;vertical-align:top;'>Last Return Date</td><td id='ppr_today' style='vertical-align:top;height:30px;'>$updatetime</td></tr>";
...
if (!mysql_num_rows($resultdoc) == 0 )
		{ 
			...
			{
			$nn++;
			echo "<td id='pp_m3_3_choose' class='th1' recno='$rowp[recno]' docno='$rowp[docno]' style='width:100px;text-align:center;font-weight:bold;border-bottom:1px solid #A6A6A6;color:#D90000;cursor:pointer;' title='View Document' >$rowp[docno] </td>";
