...
$recno++; $a = '';$b = '';$c = '';$d = '';$e = '';$f = '';$g = '';$h = '';$i = '';
		...
		//	 echo "<tr><td>test</td></tr>";
			...
			if ($i == 'O')$critical = 'X';
			  ...
			  echo "<tr><td>description $b</td><td>$c $d  barcode $q</td>";
			
			   $resultrec = mysql_query("SELECT * FROM m_sparepart where uploadno like '$uploadno'");
				if (mysql_num_rows($resultrec) == 0 ) 
					{
				
					 echo "<td>----> Insert</td></tr>";
						$sql = "INSERT INTO m_sparepart(uploadno,part_number,barcode,description,
						      remarks,maker,sptype,critical,safety_qty,usage_qty,updatetime,user)
							VALUES ('$uploadno','$c','$q','$description','$a','$e','$h','$critical','$m','$l','$now','sulong')";
							if (!mysql_query($sql))
							{
