...
session_start();
 date_default_timezone_set('Asia/Kuala_lumpur');
   ...
   $today = date("Y-m-d");
    $cyear = date("Y");
 require('db_ems.php');
echo "<div style='margin-top:20px;text-align:left;color:#007CB9;font-weight:bold;'>List of Deleted Documents</div>";
$result1 = mysql_query("SELECT * from mppr_return_header where  status = 'D'");
if (!mysql_num_rows($result1) == 0 )
{
	while($row1 = mysql_fetch_array($result1))
    {
		...
		echo "<div class='pp_m3_4_choose' docno='$row1[docno]' style='cursor:pointer;border:1px solid #63B1B1;width:250px;min-height:100px;border-radius:5px;display: inline-block;margin:20px;'>";
