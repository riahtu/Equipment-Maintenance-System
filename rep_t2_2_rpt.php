...
echo "<tr><td style='font-weight:bold;'>Report : Equipment Preventive Maintenance History Listing</td></tr>";
...
if (!mysql_num_rows($result1) == 0 )
{
	...
	while($row1 = mysql_fetch_array($result1))
    {
		...
		$result2 = mysql_query("SELECT * from t_workorder where t_workorder.equipmentid like '$row1[equipmentid]'
																
																and t_workorder.wo_type = 'PV'
																");
		...
		{	
			...
			echo "<td style='width:103px;font-weight:bold;text-align:center;border-bottom:1px solid #E9E9E9;padding-left:3px;vertical-align:top;padding-top:3px;'>Issued Quantity</td></tr>";
				
			while($row2 = mysql_fetch_array($result2))
			{
				$sparepart_name = "";
				$result3 = mysql_query("SELECT * from t_workorder_parts,mop_issue where t_workorder_parts.workorderid like '$row2[workorderid]'
																				  and mop_issue.workorderid like '$row2[workorderid]'
																						");
				if (!mysql_num_rows($result3) == 0 )
				{	
					while($row3 = mysql_fetch_array($result3))
					{
						$sparepart_name = $row3[sparepartname];
						$quantity = $row3[quantity];
					
					}
				}
				echo "<tr><td style='width:200px;border-bottom:1px solid #E9E9E9;padding-left:3px;vertical-align:top;padding-top:3px;'>$sparepart_name</td><td style='width:103px;text-align:center;border-bottom:1px solid #E9E9E9;padding-left:3px;vertical-align:top;padding-top:3px;'>$quantity</td></tr>";
