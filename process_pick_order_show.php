...
$(document).ready(function(){
	...
	$("input#p2_qty").keypress(function(e){
	 ...
	 $("input#p_barcode").keypress(function(e){
       if(e.which == 13) {
	   var workorderid = $(this).attr("workorderid");
	   var balance = $("td.s_balqty").html();
	   var p_balance = parseInt(balance);
	...
	if(data == 'Y')
	{
			...
			if(qty > 0) 
			{
				...
				for(var i=1; i < mylength;i++)
				{
					...
					var w_orderqty = $("#t_wp tr " ).eq(i).find("td.s_orderqty").html();
					var w_init_bal = $("#t_wp tr " ).eq(i).find("td.init_bal").html();
					var balance = $("#t_wp tr").eq(i).find("td.s_balqty").html();
					n_balance = parseInt(balance);
					...
					if(trim_w_barcode == trim_barcode)
					{
						...
						//pick_qty(new_qty);
							...
							$("#p_barcode").focus();
						//alert("trim_barcode "+trim_barcode+" workorderid "+workorderid);
							$.get('upd_draf_pickqty.php?barcode='+trim_barcode+'&workorderid='+workorderid+'&d_pickqty='+new_qty, function(data,status) {
								//alert(status);
							});
					} 
				//alert('done');
...
<?php
echo "<div id='sk_reselect' style='cursor:pointer;margin-top:10px;color:#FF2D2D;text-decoration:underline;text-align:left;font-family:arial;font-weight:bold;'>BACK TO SELECTION</div>";
...
//session_start();
 ...
 require('db_ems.php');
	...
	if (!mysql_num_rows($result1) == 0 )
	{
		...
		echo "<div id='title_gradient' style='text-align:left;'>Pick Parts according to Order</div>";
		echo "<table ' style='font-size:12px;font-family:arial;margin-top:10px;font-weight:bold;'>";
	
		echo "<tr><td style='width:150px;'>Requisition No </td><td>$_GET[workorderid]</td><td style='width:520px;'></td><td><button id='reset_pickqty' workorderid='$_GET[workorderid]'>Reset Pick Qty</button></td></tr>";
		...
		echo "<td style='width:150px;text-align:center;font-weight:bold;border-bottom:1px solid #A6A6A6;' > Part Number </td>";
		...
		{ 
			...
			{
			$nn++;
			echo "<td class='show_picking_doc' recno='$rowp[recno]' style='cursor:pointer;width:50px;text-align:center;font-weight:bold;border-bottom:1px solid #A6A6A6;color:#D90000;' >$rowp[docno] </td>";
		...
		while($row1 = mysql_fetch_array($result1))
		{
			...
			echo "<td class='s_partnumber' style='text-align:center;' > $row1[part_number]  </td>";
			...
			echo "<td class='s_pickqty' id='pick_qty' style='text-align:center;'> $row1[d_pickqty] </td>";
			$pickbalance -= $row1[d_pickqty];
		//	$pickbalance = $pickbalance * -1;
		//	$pick_qty = $row1[d_pickqty];
		//	$total_pickqty += $pick_qty;

			
			if ($pickbalance < 0) { $pickbalance = $pickbalance * -1; }
			if($row1[orderqty] == $iss_qty ) { $pickbalance = 0; }
			echo "<td id='s_balance' class='s_balqty' style='text-align:center;'> $pickbalance </td>";
			echo "</tr> ";
		...
		echo "<td style='text-align:center;display:none; 'id='picking_doc'> <button id='gen_pickingdoc'  workorderid='$_GET[workorderid]' style='width:200px;height:30px;'>Generate Picking Document</button></td>";
