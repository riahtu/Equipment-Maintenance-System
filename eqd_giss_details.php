...
$now = date("YmdHis");
$year = date("Y");
$result1= mysql_query("SELECT * from mop_issue_header where docno = '$_GET[stisdocno]'");
if (!mysql_num_rows($result1) == 0 )
{
	
	while($row1 = mysql_fetch_array($result1))
    {
		$wo_type = $row1[reasoncode];
		$wo_recno = $row1[recno];
...
echo "<tr><td style='width:900px;text-align:left;font-weight:bold;font-size:12px;top:20px;'>Goods Issue Details : #$_GET[stisdocno]</td><td> </td></tr>";
echo "</table>";

echo "<input type='hidden' id='stisdocno' value='$_GET[stisdocno]'/>";
...
{	
	...
	{
		//$iss_qty = mysql_result($resultrec, 0, 'quantity')* -1;

		$no++;
		echo "<tr class='wo_part_choose' >";
		echo "<td style='text-align:center;'>$no</td>";
		echo "<td >$row2[sparepartid]</td>";
		...
		echo "<td style='text-align:center;'>$row2[quantity]</td>";
		echo "</tr>";
	}
	echo "</table>";
