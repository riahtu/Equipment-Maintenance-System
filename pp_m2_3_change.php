<?php
 ...
 session_start();
 require('db_ems.php');
$result1 = mysql_query("SELECT * from mpr_receipt_header where docno = '$_GET[docno]' and status = 'N'");
...
echo "<div style='text-align:left;margin-top:20px;color:#007CB9;font-weight:bold;'>Incompleted Purchase Receipt </div>";
...
echo "<td style='width:50px;'></td><td id='pp_m1_2_docno' style='text-align:right;padding-right:50px;font-size:30px;color:#512828;height:32px;'>$_GET[docno]</td></tr>";
...
if (!mysql_num_rows($result2) == 0 )
{
	while($row2 = mysql_fetch_array($result2))
    {
		...
		echo "<tr>";
		echo "<td class='td1' style='width:200px;'><input type=text' value='$row2[barcode]' class='m12_barcode' style='width:98%;height:96%;font-size:10px;text-align:center;'/> </td>";
		echo "<td class='td1' style='width:100px;'><input type=text' value='$row2[sparepartid]' class='m12_sparepartid' style='width:96%;height:96%;font-size:10px;text-align:center;'/> </td>";
		echo "<td class='td1' style='width:325px;'><input type=text' value='$row2[sp_description]' class='m12_sparepartname' style='width:98%;height:96%;font-size:10px;'/> </td>";
		echo "<td class='td1' style='width:200px;'><input type=text' value='$maker' class='m12_maker' style='width:98%;height:96%;font-size:10px;'/> </td>";
		echo "<td  style='width:20px;background-color:#008040;' class='sp_status'>F</td>";
		echo "<td class='td1' style='width:100px;'><input type=text' value='$row2[quantity]' class='m12_rec_qty' style='width:96%;height:96%;font-size:10px;text-align:center;'/> </td>";
		echo "</tr>";
	}
}
	for ($i = $n; $i <= 10; $i += 1)
