...
session_start();
 ...
 $cyear = date("Y");
require('db_ems.php');
echo "<div style='margin-top:20px;text-align:left;color:#007CB9;font-weight:bold;'>Selections : Listing by Requisition Type </div>";
...
if (!mysql_num_rows($result1) == 0 )
{
	...
	echo "<table border=0 style='border-radius:5px;width:98%;border:1px solid #D3D3D3;padding:5px;margin-top:5px;margin-left:10px;text-align:left;font-family:Arial;font-size:11px;height:60px;'>";
		...
		echo "<td><button id='rep_t1_1_execute' program='rep_t1_1_rpt.php' sparepartid='$_GET[sparepartid]' style='width:100px;height:30px;'>Execute</button></tr>";
		echo "</table>";
