...
session_start();
 date_default_timezone_set('Asia/Kuala_lumpur');
   ...
   $today = date("Y-m-d");
    $cyear = date("Y");
 require('db_ems.php');
echo "<div style='margin-top:20px;text-align:left;color:#007CB9;font-weight:bold;'>List of Parts Purchase Return</div>";
$result1 = mysql_query("SELECT * from mppr_return_header where  status = 'C' order by docno desc");
if (!mysql_num_rows($result1) == 0 )
{
	while($row1 = mysql_fetch_array($result1))
    {
		...
		echo "<div id='pp_m3_3_choose' class='pp_m3_3_choose' docno='$row1[docno]' style='cursor:pointer;border:1px solid #63B1B1;width:250px;min-height:100px;border-radius:5px;display: inline-block;margin:20px;'>";
		echo "<table class='pr_ticket' style='width:100%;text-align:left; border-collapse: collapse;'>";
		echo "<tr><td colspan=2 style='border-bottom:1px solid #808080;height:30px;font-size:16px;color:black;font-weight:bold;background-color:lightseagreen;text-align:center;'>$row1[docno]</td></tr>";
