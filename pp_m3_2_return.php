...
$docno = $_POST[docno];
$receiveTotal = $_POST[receive_total];
...
$receiveqty = json_decode(stripslashes($_POST['receiveqty']), true);


    $result11 = mysql_query("SELECT * FROM m_transtype_nr where company = '$_SESSION[company]' and transtype = 'PPR' and year = '$docyear'");
	...
	$sql = "INSERT INTO mppr_return_header(docno,company,storeid,purchaseorderno,deliveryorderno,supplierid,docdate,
	...
	for($i=0;$i<$size;$i++)
	{
		...
		$sql = "INSERT INTO mppr_return(docno,company,storeid,lineno,sparepartid,barcode,sp_description,receive_qty,quantity,createtime,userid)
			VALUES ('$new_docno','$_SESSION[company]', '$_SESSION[storeid]','$lineno', '$spid','$bc',
			                           '$desc','$r_qty','$qty','$now','$_SESSION[userid]')";
			if (!mysql_query($sql)) { die('Error: ' . mysql_error());$error_found = 'X'; }
	...
	$sqlupd1 = "update m_transtype_nr set currentno='$nextno',lastupdate = '$now',userid='$_SESSION[userid]'
			where company = '$_SESSION[company]' and  transtype = 'PPR' and year = '$docyear'";
	if (!mysql_query($sqlupd1))
	{
		die('Error: ' . mysql_error());                      
	}

	if($receiveTotal == $newTotal)
	{
		$sqlupd2 = "update mpr_receipt_header set status = 'P', updatetime = $now where company = '$_SESSION[company]' and docno = '$docno'";
