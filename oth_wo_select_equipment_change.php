...
$resultrec = mysql_query("SELECT * FROM m_equipment_sparepart where equipmentid = '$_GET[equipmentid]'   ");
	...
	{	
		echo "<table id='wo_eq_part_list' style='margin-top:10px;text-align:left;'>";
		 echo "<tr>";
		  ...
		  echo "<td style='border-bottom:1px solid #51A2A2;width:50px;text-align:center;'>Spare Id</td>";
		  echo "<td style='border-bottom:1px solid #51A2A2;width:300px;padding-left:3px;'>Spare Description</td>";
		...
		while($rowrec = mysql_fetch_array($resultrec))
		{
		  ...
		  echo "<td style='text-align:center;'><input type='text' class='wo_orderqty' value='$orderqty' style='width:40px;height:20px;text-align:center;'/> </td>";
		  echo "</tr>";
		}
		echo "</table>";
	...
	echo "<div id='wo_save_show' style='margin-top:40px;margin-bottom:20px;'>";
		echo "<table>";
		echo "<tr>";
		echo "<td style='width:150px;'></td>";
		echo "<td><button id='mm_oth_wo_active' style='width:200px;height:30px;'>Cancel</button></td>";
		echo "<td style='width:30px;'></td>";
		echo "<td><button id='oth_wo_change_save' style='width:200px;height:30px;'>Save</button></td>";
		echo "</tr>";
		echo "</table>";
	echo "</div>";
	...
	echo "<div style='font-size:25px;color:#FF0000;margin-top:20px;'>Please define Assignment Equipment ---> Sparepart</div>";
